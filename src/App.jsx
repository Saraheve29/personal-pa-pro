...
export default function App(){
  ...
  const PARSE_SYS=`Extract calendar events. Return ONLY valid JSON, no markdown: {"events":[{"title":string,"date":"YYYY-MM-DD","time":"HH:MM","priority":"critical|high|medium|low","notes":string}],"summary":string}. Today is ${fmt(today)}. Default date:today, time:09:00.`;
  ...
  async function parsePaste(){if(!pasteText.trim()||pasteBusy)return;setPasteBusy(true);setPasteRes(null);try{const raw=await callAI({system:PARSE_SYS,messages:[{role:"user",content:pasteText}]});setPasteRes(JSON.parse(raw.replace(/```json|```/g,"").trim()));}catch{setPasteRes({error:true});}setPasteBusy(false);}
  ...
  async function parseImg(){if(!imgFile||imgBusy)return;setImgBusy(true);setImgRes(null);try{const b64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(",")[1]);r.onerror=rej;r.readAsDataURL(imgFile);});const raw=await callAI({system:PARSE_SYS+" Perform OCR on image.",messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:imgFile.type||"image/jpeg",data:b64}},{type:"text",text:"Extract all appointments or events from this image."}]}]});setImgRes(JSON.parse(raw.replace(/```json|```/g,"").trim()));}catch{setImgRes({error:true});}setImgBusy(false);}
  ...
  function ResultPreview({result,onAdd,onDiscard}){
    ...
    if(result.error)return<div style={{border:`1px solid ${C.crimson}`,background:C.crimsonBg,padding:"14px 16px",marginTop:8,fontSize:13,color:C.crimson,fontFamily:FB,borderRadius:4}}>Unable to extract events. Please try again.</div>;
  ...
  const ImportView=()=>(
    <div>
      ...
      {impTab==="image"&&<div>
        <div style={{fontSize:13,color:C.inkLight,fontFamily:FB,lineHeight:1.75,marginBottom:14}}>Upload a screenshot, booking confirmation, or photograph. Eleanor will read and extract all relevant appointments.</div>
        <label style={{display:"block",border:`1.5px dashed ${C.goldBorder}`,padding:"32px 20px",textAlign:"center",cursor:"pointer",marginBottom:12,background:C.cardWarm,color:C.gold,fontSize:13,fontFamily:FB,letterSpacing:"0.06em",borderRadius:6}}>
          {imgPrev?"✦ Image ready — tap Extract below":"Tap to select image or screenshot"}
  ...
  return(
    <div style={{fontFamily:FB,background:C.parchment,minHeight:"100vh",maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column",color:C.ink}}>
      ...
      {view!=="chat"&&<div style={{padding:"11px 24px",borderTop:`1px solid ${C.border}`,background:C.cream,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{fontSize:9,color:C.inkFaint,fontFamily:FM,letterSpacing:"0.22em",textTransform:"uppercase"}}>Executive Assistant · Private Service</div>
        <div style={{fontSize:11,color:C.goldBorder,fontFamily:FD}}>✦</div>
      </div>}
    </div>
  );
}
