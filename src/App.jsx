// VERSION_CHECK: Briefing-Fresh-And-School-Fix build - July 2 2026 v73
  ...
  async function sendChat(directText,imageB64,imageMime){
    ...
    try{
      ...
      const contextContent=[
        ...
        "ACTIVE TIME AWARENESS — the current time right now is "+timeStr+". Before making ANY timing suggestion, compare the current time against each appointment's time: anything with a time EARLIER than "+timeStr+" today has ALREADY HAPPENED — refer to it in the past tense and NEVER tell Sarah to 'prepare for' or 'rest before' it. For a delivery or appointment with a window (e.g. 1pm-2pm), if the current time is past the end of that window, it is DONE. Example: if it is "+timeStr+" and the food delivery was 1pm-2pm, that delivery has already arrived — say so, and move on to what is still to come. Only use 'this afternoon/later/coming up' for things whose time is still AHEAD of "+timeStr+".",
        "TRUST SARAH'S LIVED REALITY — when Sarah tells you something has happened (e.g. 'my delivery arrived', 'I'm back from Wisbech', 'I've taken my meds'), accept it immediately as fact. NEVER contradict her, never tell her to prepare for something she just said is done, and never question whether she is really experiencing what she describes. She knows her own day better than any schedule entry.",
